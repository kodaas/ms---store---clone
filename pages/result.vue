<template>
    <section class="p-5 md:p-10 space-y-10">
        <div class="flex justify-between items-center">
            <h1 class="text-2xl text-foreground-display">"{{ $route.query?.q }}"</h1>

            <Button @click="showDropdown = !showDropdown" intent="ghost"
                custom-class="text-foreground-display active:bg-background-50" size="sm">
        ...
        <div v-else class="flex gap-3 flex-wrap">
            <Button v-for="(category, index) in categories" :key="index" @click="selectedCategoryIndex = index"
                intent="outline" size="sm"
                :custom-class="selectedCategoryIndex === index ? 'rounded-full border border-primary text-primary dark:border-primary-light dark:text-primary-light text-sm hover:bg-transparent px-7' : 'rounded-full border text-sm px-7'">{{
                    category }}</Button>
        </div>

        <div class="relative grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 pb-20">
            <SearchCard v-for="item in 10" />
            ...
            <div
                class="absolute bg-gradient-to-b from-transparent to-background-200 w-full pt-20 pb-5 bottom-5 left-0 text-center text-lg">
